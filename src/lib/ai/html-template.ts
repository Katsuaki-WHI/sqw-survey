 ...
 *
 * 【重要ルール】プロンプト改善時は必ずこのファイルも確認・更新すること
 * カテゴリ名は必ず「ワゴン部位（カテゴリ名）」形式で統一：
 *   景色（ミッション）／道筋（戦略）／ロープ（リーダーシップ）／タイヤ（仕組み）
 *   押す人の体（能力・意欲）／押す人の態度（風土・文化）／積荷（リソース・強み）
 *   登場人物の多彩さ（多様性）／幸福度
 * AIプロンプト（whi-philosophy.ts）とラベル定義（questions.ts）と
 * このHTMLテンプレートの3点セットを常に同時に更新すること
 */
...
【カテゴリ名の表記ルール】
カテゴリ名は必ず「ワゴン部位（カテゴリ名）」形式で表記すること：
...
<div style="background:linear-gradient(135deg,#0D2137,#028090);color:white;border-radius:16px;padding:32px;text-align:center;margin-bottom:24px;">
  <div style="font-size:12px;color:#02C39A;letter-spacing:2px;">SQWサーベイ2 AIレポート</div>
...
<div style="background:#FFFBEB;border:2px solid #F0A500;border-radius:12px;padding:20px;margin:16px 0;">
  <div style="font-size:12px;color:#64748B;">メンバー間の景色の違い</div>
...
Always use "Wagon Part (Category Name)" format:
...
- Gold-bordered "Perception Gap" highlight boxes (labeled "Different Perspectives Among Members")
