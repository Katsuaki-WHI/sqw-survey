...
個人レポートが提供する価値：
・個人の「幸福度」に影響している要因を明らかにする
・自分の強みが活かされているかを言語化する
・チームとの認識ギャップを「発見」として提示する
...
・ギャップを示す際は「このギャップがあなたにどんなストレスをもたらしているか」
　「これが改善されるとあなたにどんな恩恵があるか」まで踏み込む
・読み終わった後に「よし、やってみよう」と前向きになれる言葉を選ぶ
...
4. 「このギャップがあなたにもたらしているストレス」を具体的に言語化
...
# 【セクション3】あなたの「幸福と成果」の現在地
Q26（幸福度）とQ02（ミッション・ビジョン）のスコアから以下の4タイプのどれかに分類して詳細解説：

・エンゲージ型（幸福度高・ミッション高）：
「今のあなたは、自分らしく力を発揮できている状態です。幸福と成果が両立しているこの状態を、ぜひ周囲にも広げていきましょう。」

・理念先行型（幸福度低・ミッション高）：
「あなたはこのチームを誰より信じているのに、報われていない感覚があるかもしれません。その想いの強さこそ、あなたがこのチームに欠かせない理由です。その気持ちを、少しずつ言葉にして届けていきましょう。」

・実行先行型（幸福度高・ミッション低）：
「今の仕事自体は楽しい。でも『なんのためにやっているのか』という問いが心のどこかにあるかもしれません。その問いを持てること自体、あなたが成長している証です。」

・離脱リスク型（幸福度低・ミッション低）：
「今のあなたには、少し立ち止まる時間が必要かもしれません。これは弱さではありません。あなたが感じている苦しさは、あなたが本気でこのチームに向き合ってきた証です。小さな一歩から、一緒に考えていきましょう。」
